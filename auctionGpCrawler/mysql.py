...
class Mysql:
    ...
    def __init__(self):
        try:
			self.db = MySQLdb.connect('localhost', 'root', 'root', 'gp_info',3306)
    ...
    def insertData(self,jsonDatas):
        try:
            ...
            if self.cur._rows[0][0] == 0:
                insertInfo = "insert into %s (%s) VALUES (%s)" % ('gp_auction_info', "auction_id,court_id,title,kind_id,url,start_price,current_price,cash_deposit,payment_advance,access_price,fare_increase,paimai_times,paimai_type,delay_cycle,corporate_agent,phone,selling_period,online_cycle,bidding_record,paimai_model,enrollment,set_reminders,onlookers,data_time,paimai_status,main_status",str(jsonDatas['auction_id'])+",'"+str(jsonDatas['court_id'])+"','"+ str(jsonDatas['title'])+"','"+ str(jsonDatas['kind_id'])+"','"+str(jsonDatas['url'])+"','"+str(jsonDatas['start_price'])+"','"+str(jsonDatas['current_price'])+"','"+str(jsonDatas['cash_deposit'])+"','"+str(jsonDatas['payment_advance'])+"','"+str(jsonDatas['access_price'])+"','"+str(jsonDatas['fare_increase'])+"','"+str(jsonDatas['paimai_times'])+"','"+str(jsonDatas['paimai_type'])+"','"+str(jsonDatas['delay_cycle'])+"','"+str(jsonDatas['corporate_agent'])+"','"+str(jsonDatas['phone'])+"','"+str(jsonDatas['selling_period'])+"','"+str(jsonDatas['online_cycle'])+"','"+str(jsonDatas['bidding_record'])+"','"+str(jsonDatas['paimai_model'])+"','"+str(jsonDatas['enrollment'])+"','"+str(jsonDatas['set_reminders'])+"','"+str(jsonDatas['onlookers'])+"','"+str(jsonDatas['data_time'])+"','"+str(jsonDatas['circle'])+"','1'")
                print insertInfo
                try:
